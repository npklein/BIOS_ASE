...
aCount <- read.table('/groups/umcg-bios/tmp03/projects/outlierGeneASE/binomialTest/genotypes_BIOS_LLDeep_Diagnostics_merged_phasing_noRnaEditing.aCount.txt', header=T, check.names=FALSE)
bCount <- read.table('/groups/umcg-bios/tmp03/projects/outlierGeneASE/binomialTest/genotypes_BIOS_LLDeep_Diagnostics_merged_phasing_noRnaEditing.bCount.txt',header=T, check.names=FALSE)
annotations <- as.data.frame(fread("sample_genes_outliers_phenotypes.removedCODAM.txt"))
...
for(gene in unique(aCount$ENSEMBLID)){
 ...
 # ab_count_all <- rbind(ab_count_all, ab_count)
  ...
  write.table(ab_count, paste0('genes/',gene,'.txt'))
}  
...
write.table(abcount_all_genes, paste0('binom_ASE_all_genes.txt'), quote = F, sep='\t')
