...
carriers_per_disease_snpInfo <- merge(carriers_per_disease, snpInfo, by='snp',fill=T)
...
outliers_total$fraction_outlier <- outliers_total$outlier/(outliers_total$outlier+outliers_total$not_outlier)
print(outliers_total)
outliers_total$ClinSimple <- factor(outliers_total$ClinSimple, levels=c('Pathogenic',
                                                                    'Uncertain significance','Benign'))
print(outliers_total)
ggplot(outliers_total, aes(ClinSimple, fraction_outlier, fill=ClinSimple))+
  geom_bar(stat='identity')+
  theme_bw(base_size = 18)+
  scale_fill_brewer(palette="Dark2")+
  ylab('Fraction of SNPs that is an outlier')+
