...
open(OUTPUT2, "> /groups/umcg-bios/tmp03/projects/outlierGeneASE/variantPenetranceAndPLIAnalysis/counts.chrALL.txt") || die "Can't open output file for chrALL!\n";
    
print OUTPUT2 "VARIANT\tCHR\tPOS\tRSID\tREF\tALT\tSNPEFFANNOTATION\tSNPEFFIMPACT\tSNPEFFGENE\tVEPCONSEQUENCE\tVEPIMPACT\tVEPFEATURETYPE\tVEPBIOTYPE\tGONLAF\tEXACAF\tPLI\tAF\t";
print OUTPUT2 "PLISTATUS\tGENEID\tGENENAME\tCGDINHERITANCE\tCGDAGEGROUP\tCGDMANIFESTATION\tCLNVRSIG\t";
print OUTPUT2 "NonOutlRefCount\tNonOutlAltCount\tOutlRefCount\tOutlAltCount\tNonOutlSampleCount\tOutlSampleCount\t";
print OUTPUT2 "UpdNonOutlRefCount\tUpdNonOutlAltCount\tUpdOutlRefCount\tUpdOutlAltCount\n";
...
for (my $CHR=1; $CHR<=22; $CHR++){
            ...
            #If gene exists in genes hash continue, otherwise don't process it at all
                ...
                for (my $j=0; $j<=$#variantsToProcess; $j++){
                    ...
                    if (exists $binomStatusHash{ $sample }{ $geneID }) {
                        ...
                        #Check if sample exists in outlierHash, if so add ref and alt counts to new hash.
                        #This new hash will later be used to construct two seperate tables
                        if (exists $outlierHash{ $sample }{ $geneID }) {
                            if ($ref eq $allHapA) { #if reference allele matches hapA allele, add it here, otherwise add it to alt count
    ...
    open(OUTPUT, "> /groups/umcg-bios/tmp03/projects/outlierGeneASE/variantPenetranceAndPLIAnalysis/counts.chr$CHR.txt") || die "Can't open output file for chr$CHR!\n";
    ...
    print OUTPUT "NonOutlRefCount\tNonOutlAltCount\tOutlRefCount\tOutlAltCount\tNonOutlSampleCount\tOutlSampleCount\t";
    print OUTPUT "UpdNonOutlRefCount\tUpdNonOutlAltCount\tUpdOutlRefCount\tUpdOutlAltCount\n";
    ...
    foreach my $key (sort keys %annotations){
        ...
        if (exists $varRefCount{ $variant }{ YES }) { #Counts for this variant exist, so print it to output
            ...
            print OUTPUT "$nonoutlierVarRefCount\t$nonoutlierVarAltCount\t$outlierVarRefCount\t$outlierVarAltCount\t$nonoutlierVarStatusCount\t$outlierVarStatusCount\t";
            print OUTPUT "$updNonoutlierVarRefCount\t$updNonoutlierVarAltCount\t$updOutlierVarRefCount\t$updOutlierVarAltCount\n";
            
            print OUTPUT2 "$variant\t$annotation\t$af\t$pliStat\t$geneID\t$geneName\t$inheritance\t$ageGroup\t$manifestation\t$clinvar\t";
            print OUTPUT2 "$nonoutlierVarRefCount\t$nonoutlierVarAltCount\t$outlierVarRefCount\t$outlierVarAltCount\t$nonoutlierVarStatusCount\t$outlierVarStatusCount\t";
            print OUTPUT2 "$updNonoutlierVarRefCount\t$updNonoutlierVarAltCount\t$updOutlierVarRefCount\t$updOutlierVarAltCount\n";
...
open(MAJOR, "> /groups/umcg-bios/tmp03/projects/outlierGeneASE/variantPenetranceAndPLIAnalysis/counts.matrix.majorAllelle.chrALL.txt") || die "Can't open output file for major allele!\n";
open(MINOR, "> /groups/umcg-bios/tmp03/projects/outlierGeneASE/variantPenetranceAndPLIAnalysis/counts.matrix.minorAllelle.chrALL.txt") || die "Can't open output file for minor allele!\n";
shift(@BINOMheader);
