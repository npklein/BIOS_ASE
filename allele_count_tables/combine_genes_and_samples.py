...
print('read outliers')
gene_outlier = read_gene_outliers()


gene_outlier_sum = {}

with open('/groups/umcg-bios/tmp03/projects/outlierGeneASE/infoTables/alleleCountPerGroupPerGene.outliers.binom.txt','w') as out:
...
with open('/groups/umcg-bios/tmp03/projects/outlierGeneASE/infoTables/alleleCountPerGroupPerGene.not_outliers.binom.txt','w') as out:
    out.write('gene\tsnp\tAF\tref\talt\t0\t1\t2\ttotal\n')
    # per gene, sum for the outlier and not_outlier group the allele counts. Divide by number of sampls (that have logFC count) to get normalized number of alleles per gene
    for gene in gene_outlier:
        for g in gene.split(';'):
