...
for (my $j=1; $j<=$#outliers; $j++){
    ...
    if ($array[1] eq $nonSNP) { #Check if SNP on current line is the exact same SNP as on the line from other file
        if ($totNonAlt > 0 || $totAlt > 0) {
            print OUTPUT "$uniq\t$chr\t$pos\t$ref\t$alt\t$geneID\t$af\t$nonTotal\t$nonHomRef\t$nonHet\t$nonHomAlt\t$totNonAll\t$totNonRef\t$totNonAlt\t$annotation\tnon-outlier\n";
            print OUTPUT "$uniq\t$chr\t$pos\t$ref\t$alt\t$geneID\t$af\t$total\t$homRef\t$het\t$homAlt\t$totAll\t$totRef\t$totAlt\t$annotation\toutlier\n";
        }
    }else{
