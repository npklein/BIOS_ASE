...
carriers_per_clinvar <- fread('carriers_per_disease/carriers_per_disease.hetsOnly.txt')
...
clinvarTable <- fread('clinvarSNPs.txt')
...
ggsave('figures/proportion_outlier_per_clinvar_per_disease.png',width=25, height=20)
...
#  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  ...
  scale_y_continuous(limit=c(0,1.1))+
  guides(fill=FALSE)
ggsave('figures/proportion_outlier_per_clinvar.png',width=12, height=8)
